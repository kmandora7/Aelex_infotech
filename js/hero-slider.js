...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const SLIDE_DURATION = 6000; // 6.0 seconds per slide
  ...
  if (mobileToggle && mobileDrawer) {
    ...
    // Close mobile drawer when clicking any nav link (including main product link)
  ...
  if (prodTrack && prodViewport) {
    ...
    function updateNavButtons() {
      const maxScroll = prodViewport.scrollWidth - prodViewport.clientWidth;
      if (prodPrevBtn) {
        prodPrevBtn.disabled = prodViewport.scrollLeft <= 10;
        prodPrevBtn.classList.toggle('disabled', prodViewport.scrollLeft <= 10);
      }
      if (prodNextBtn) {
        prodNextBtn.disabled = prodViewport.scrollLeft >= maxScroll - 10;
        prodNextBtn.classList.toggle('disabled', prodViewport.scrollLeft >= maxScroll - 10);
      }
    }
    ...
    function updateProdSlider(targetIndex) {
      ...
      if (cards.length > 0) {
        ...
        const targetScrollLeft = currentProdIndex * (cardWidth + gap);
        prodViewport.scrollTo({ left: targetScrollLeft, behavior: 'smooth' });
      }
    ...
    prodViewport.addEventListener('scroll', () => {
      if (cards.length > 0) {
        const cardWidth = cards[0].getBoundingClientRect().width;
        const gap = parseFloat(window.getComputedStyle(prodTrack).gap) || 28;
        const calculatedIndex = Math.round(prodViewport.scrollLeft / (cardWidth + gap));
        if (calculatedIndex !== currentProdIndex) {
          currentProdIndex = calculatedIndex;
          if (prodDotsWrap) {
            const dots = Array.from(prodDotsWrap.children);
            dots.forEach((dot, idx) => {
              dot.classList.toggle('active', idx === currentProdIndex);
